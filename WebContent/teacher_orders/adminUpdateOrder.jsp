       ...
       <!-- Page Top Navigation  -->
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
          ...
          <div class="row">
			...
			<form method="post"  action="TeacherOrderController">
			Teacher:
       		<c:forEach items="${teachers}" var="teacher">
					<c:choose>
						<c:when test="${teacherProduct.teacherid == teacher.teacherid }">
						<input type="text" class="form-control" name="teacherid" value="<c:out value="${teacher.name}" />" readonly/>
						</c:when>
					</c:choose>
			</c:forEach><br>
       		Product:
       		<c:forEach items="${products}" var="product">
					<c:choose>
						...
						<input type="text" class="form-control" name="" value="<c:out value="${product.prodName}" />" readonly/>
						</c:when>
					</c:choose>
			</c:forEach>
			<br>Status:
              	<select name="status" class="form-control border">
                <option selected hidden value="<c:out value="${teacherProduct.status}"/>"><c:out value="${teacherProduct.status}"/></option>
				<option value="Pending">Pending</option>
				<option value="Completed">Completed</option>
			</select>
			<br>Quantity:<input type="text" class="form-control" name="orderProdQuantity" pattern="^[0-9]*$" title="Number only" value="<c:out value="${teacherProduct.quantity}" />" readonly/>	
			...
			<input type="text" class="form-control" name="prodid" value="<c:out value="${teacherProduct.prodid}" />" hidden/>
