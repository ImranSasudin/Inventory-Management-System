...
$(document).ready(function(){
	...
	$("#add").click(function (e){
		...
		$("#items").append('<div class="items">Product ID: <br><select class="autocomplete form-control" name="prod" required>' + 
				'<c:forEach items="${products}" var="product">' +
				'<option value="<c:out value="${product.prodid}"/>"><c:out value="${product.prodName}"/>   (ID : <c:out value="${product.prodid}"/>)</option></c:forEach> </select> <br><br>' +
				'Quantity :<br> <input type="text" class="form-control" name="orderProdQuantity" required value="0"> <input type="button" value="Delete" id="delete" class="btn btn-danger align-right"></div><br><br>');
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
          ...
          <div class="row">
			...
			<%	String email = (String)session.getAttribute("currentSessionUser");%>
			 <form action = "/Inventory/OrderProductController" method = "post">
				...
				<c:forEach items="${products}" var="product">
					<option value="<c:out value="${product.prodid}"/>"><c:out value="${product.prodName}"/>   (ID : <c:out value="${product.prodid}"/>)   RM<c:out value="${product.prodPrice}"/> per unit</option>
				</c:forEach>
				</select> <br><br>
				Quantity :<br>
				<input type = "number"  class="form-control" name="orderProdQuantity" pattern="^[0-9]*$" title="Number only" required><br>
				</div>
				</div>
