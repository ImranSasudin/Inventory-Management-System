       ...
       <!-- Page Top Navigation  -->
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
          ...
          <div class="row">
			...
			<form action = "/Inventory/OrderController" method = "post">
			<%	String email = (String)session.getAttribute("currentSessionUser");%>
			Order ID: <br>
			<input type = "text" class="form-control" name="orderid" value="<c:out value="${orderProduct.orderid}"/>" readonly/><br>
			Product ID: <br>
			<input type = "text" class="form-control" name="prodid" value="<c:out value="${orderProduct.prodid}" />" readonly><br>
			Status :<br> 
			<select name="status" class="form-control" required>
		 	<option selected hidden value="<c:out value="${orderProduct.status}" />"><c:out value="${orderProduct.status}" /></option>
		 	<option value="Received">Received</option>
			</select><br>
			Quantity :<br>
			<input type = "text" class="form-control" name="orderProdQuantity" value="<c:out value="${orderProduct.orderProdQuantity}" />" readonly><br>
